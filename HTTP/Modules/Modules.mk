...
$(HTTP_MOD_HELLO_OUT): $(HTTP_MOD_HELLO_OBJ)
	mkdir -p $(dir $@)
	$(LD) -o $(HTTP_MOD_HELLO_OUT) $(LD_FLAGS) -shared $(HTTP_MOD_HELLO_OBJ)
...
$(HTTP_MOD_HEADER_OUT): $(HTTP_MOD_HEADER_OBJ)
	mkdir -p $(dir $@)
	$(LD) -o $(HTTP_MOD_HEADER_OUT) $(LD_FLAGS) -shared $(HTTP_MOD_HEADER_OBJ)
...
LUA_CC_FLAGS = $(shell pkg-config --silence-errors --cflags lua) $(shell pkg-config --silence-errors --cflags lua5.2)
LUA_LD_FLAGS = $(shell pkg-config --silence-errors --libs lua) $(shell pkg-config --silence-errors --libs lua5.2)

$(OBJ_DIR)/$(HTTP_MOD_LUA_DIR)/%.o: $(HTTP_MOD_LUA_DIR)/%.cpp
	mkdir -p $(dir $@)
	$(CXX) $(CC_FLAGS) $(HTTP_SRV_CC_FLAGS) $(LUA_CC_FLAGS) -fPIC -g -c -o $@ $<

$(HTTP_MOD_LUA_OUT): $(HTTP_MOD_LUA_OBJ)
	mkdir -p $(dir $@)
	$(LD) -o $(HTTP_MOD_LUA_OUT) $(LD_FLAGS) $(LUA_LD_FLAGS) -shared $(HTTP_MOD_LUA_OBJ)
